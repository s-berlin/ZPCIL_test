...
void GEOM(int NW)
{
    ...
    if (U >= 1) {
     ...
     m32:   W[i] = (PI * (ZN[i] - 0.5) + 2 * X[i] * TGAL + Z[i] * INVAT) * MN * CSAL;
        ...
        if (IPR >= 3) {
            f_1 << "\nТОЛЩИНА ПО ХОРДЕ ЗУБА ДЕЛИТЕЛЬНАЯ     S     " << round(SY[0] * 1000) / 1000 << "    " << round(SY[1] * 1000) / 1000;    // 5.099    4.119  вместо   5.274   4.261 ???
            f_1 << "\nВЫСОТА ДО ХОРДЫ ЗУБА ДЕЛИТЕЛЬНАЯ     HA     " << round(HAY[0] * 1000) / 1000 << "    " << round(HAY[1] * 1000) / 1000;  // получились значения на 0.001 меньшие
        }
        /*
  DO 37 I = 1, 2
        C      DY(I) = DA(I) - 2 * M
