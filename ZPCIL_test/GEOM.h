...
void GEOM(int NW)
{
    ...
    if (AW == 0) {
        ...
        f_1 <<     "\nМЕЖОСЕВОЕ РАССТОЯНИЕ                       AW " << setw(10) << AW;

        ZPRGMS(ALFTW, GRM, SEC);
        if (IPR >= 3) {
            f_1 << "\nПЕРЕДАТОЧНОЕ ЧИСЛО                          U " << setw(10) << UU;
            f_1 << "\nУГОЛ ЗАЦЕПЛЕНИЯ (ГРАД.МИН.СЕК)          ALFTW " << setprecision(2) << fixed << setw(8) << GRM << ".";
        ...
        if (flaw == 0) f_1 << "\nМЕЖОСЕВОЕ РАССТОЯНИЕ                 AW " << setw(10) << AW;
     //   if (flaw > 0)  
            f_1 <<            "\nКОЭФФИЦИЕНТ СУММЫ СМЕЩЕНИЙ           XS " << setw(10) << round(XS * 1000) / 1000;
        ...
        if (X1 > -5.) X[1] = XS - X1;
            ...
            if (X2 > -5) X[0] = XS - X2;
                ...
                if (XS < -0.5 || XS > 2) {
                    f_1 << "\nКОЭФФИЦИЕНТ СУММЫ СМЕЩЕНИЙ            XS " << setw(10) << round(XS * 1000) / 1000 << endl;
        ...
        f_1 << "\nКОЭФФИЦИЕНТЫ СМЕЩЕНИЯ                 X " << setw(10) << round(X[0] * 1000) / 1000 << setw(10) << round(X[1] * 1000) / 1000;
    ...
    if (U >= 1) {
        ...
        f_1 << "\nДЕЛИТЕЛЬНЫЕ ДИАМЕТРЫ                  D " << setprecision(3) << fixed << setw(10) << D[0] << setw(10) << D[1];
        f_1 << "\nНАЧАЛЬНЫЕ ДИАМЕТРЫ                   DW " << setw(10) << DW[0] << setw(10) << DW[1];
        f_1 << "\nДИАМЕТРЫ ВЕРШИН                      DA " << setw(10) << DA[0] << setw(10) << DA[1];
        f_1 << "\nДИАМЕТРЫ ВПАДИН                      DF " << setw(10) << DF[0] << setw(10) << DF[1];
        ...
        if (IPR >= 3) f_1 << "\n\nКОЭФФ. ТОРЦОВОГО ПЕРЕКРЫТИЯ       EPALF " << setprecision(3) << fixed << setw(14) << EPALF;
        ...
        if (BE != 0) {
            ...
            if (BE != 0 && EPBET >= 1 && IPR >= 3)  f_1 << "\nКОЭФФ. ОСЕВОГО ПЕРЕКРЫТИЯ         EPBET " << setw(14) << EPBET;
            if (BE != 0 && EPBET < 1 && IPR >= 3)   f_1 << "\nКОЭФФ. ОСЕВОГО ПЕРЕКРЫТИЯ EPBET (МЕНЬШЕ 1) " << setw(11) << EPBET;
        ...
        if (BE == 0) {
            ...
            if (DELZV != 0) {
                ...
                if (IPR >= 3) {
                    ...
                    f_1 << "\nПРИ МОДИФИКАЦИИ ПРОФИЛЯ ЗУБЬЕВ     EPAM " << setw(13) << EPAM;
                }
            }
        ...
        cout << "GEOM: after CNT1:  II = " << II << "   EPALF = " << EPALF << "   EPBET = " << EPBET << endl;  // организовать прерывывание, если II == 3:    IF(II.EQ.3)  GOTO 61
        f_7 << "GEOM: after CNT1:  II = " << II << "   EPALF = " << EPALF << "   EPBET = " << EPBET << "   IPR = " << IPR << "   II = " << II << endl;  // организовать прерывывание, если II == 3:    IF(II.EQ.3)  GOTO 61
        if (II == 3) f_7 << "GEOM:   Плохое качество зацепления. Таблица параметров не выводится." << endl;  // организовать прерывывание, если II == 3:    IF(II.EQ.3)  GOTO 61
        ...
        if (II != 3) {
            ...
            if (IPR >= 3) {
                ...
                f_1 << "\nПОСТОЯННАЯ ХОРДА ЗУБА                SC " << setw(9) << round(SC[0] * 1000) / 1000 << setw(9) << round(SC[1] * 1000) / 1000;
                f_1 << "\nВЫСОТА ДО ПОСТОЯННОЙ ХОРДЫ           HC " << setw(9) << round(HH[0] * 1000) / 1000 << setw(9) << round(HH[1] * 1000) / 1000 << endl;
            }
            ...
            if (IPR > 3) {
                ...
                if (IIW1 > 0 && IIW2 == 0 && IPR >= 3) {
                    f_1 << "\nДЛИНА ОБЩЕЙ НОРМАЛИ                   W " << setw(9) << W[1];
                    f_1 << "\nЧИСЛО ЗУБЬЕВ В ДЛИНЕ ОБЩЕЙ НОРМАЛИ   ZW " << setprecision(0) << fixed << setw(9) << ZN[1];
                }
                if (IIW2 > 0 && IIW1 == 0 && IPR >= 3) {
                    f_1 << "\nДЛИНА ОБЩЕЙ НОРМАЛИ                   W " << setprecision(3) << fixed << setw(9) << W[0];
                    f_1 << "\nЧИСЛО ЗУБЬЕВ В ДЛИНЕ ОБЩЕЙ НОРМАЛИ   ZW " << setw(9) << ZN[0];
                }
                if (IIW1 == 0 && IIW2 == 0 && IPR >= 3) {
                    f_1 << "\nДЛИНА ОБЩЕЙ НОРМАЛИ                   W " << setw(9) << round(W[0] * 1000) / 1000 << setw(9) << round(W[1] * 1000) / 1000;
                    f_1 << "\nЧИСЛО ЗУБЬЕВ В ДЛИНЕ ОБЩЕЙ НОРМАЛИ   ZW " << setw(9) << ZN[0] << setw(9) << ZN[1];
                }
            }
            ...
            if (IPR >= 3) {
                f_1 << "\nТОЛЩИНА ПО ХОРДЕ ЗУБА ДЕЛИТЕЛЬНАЯ     S " << setw(9) << round(SY[0] * 1000) / 1000 << setw(9) << round(SY[1] * 1000) / 1000;
                f_1 << "\nВЫСОТА ДО ХОРДЫ ЗУБА ДЕЛИТЕЛЬНАЯ     HA " << setw(9) << round(HAY[0] * 1000) / 1000 << setw(9) << round(HAY[1] * 1000) / 1000 << endl;  // получились значения на 0.001 меньшие
            }
            ...
            cout << "TH = " << TH[0] << "    " << TH[1] << endl;  // 0.11   0.11 неверно,  должно быть 0.2    0.2   - см. DOP1L
            ...
            if (IPR >= 3) { //WRITE(1, 219) DLTHMN, DLTHMX
                ...
                f_1 << "\nЦИАЛЬНОГО ЗУБОМЕРА                 DLTH " << setw(9) << round(DLTHMN[0] * 1000) / 1000 << setw(9) << round(DLTHMN[1] * 1000) / 1000;  // 0.2    0.25    вместо   0.2      0.25
            ...
            if (IDXS == 1 && BE != 0 && IPR > 6) {
                ...
                f_1 << "\nОТ -0.5 ДО 0.5     XS = " << setw(6) << XS << endl;
                f_1 << "\nРЕКОМЕНДУЕТСЯ ИЗМЕНИТЬ ПАРАМЕТРЫ ПЕРЕДАЧИ (ЧИСЛО ЗУБЬЕВ,УГОЛ НАКЛОНА)" << endl;
