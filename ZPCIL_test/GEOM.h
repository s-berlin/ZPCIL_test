...
void GEOM(int NW)
{
    ...
    if (AW == 0) {
        ...
        f_1 <<     "\nМЕЖОСЕВОЕ РАССТОЯНИЕ                       AW " << setw(13) << AW;

        ZPRGMS(ALFTW, GRM, SEC);
        if (IPR >= 3) {
            f_1 << "\nПЕРЕДАТОЧНОЕ ЧИСЛО                          U " << setw(13) << UU;
            f_1 << "\nУГОЛ ЗАЦЕПЛЕНИЯ (ГРАД.МИН.СЕК)          ALFTW " << setprecision(2) << fixed << setw(15) << GRM << "." << setw(2) << SEC << endl;
        }
     }
        ...
        if (flaw == 0) f_1 << "\nМЕЖОСЕВОЕ РАССТОЯНИЕ                 AW " << setw(14) << AW;
     //   if (flaw > 0)  
            f_1 <<            "\nКОЭФФИЦИЕНТ СУММЫ СМЕЩЕНИЙ           XS " << setw(14) << round(XS * 1000) / 1000;
        ...
        if (X1 > -5.) X[1] = XS - X1;
            ...
            if (X2 > -5) X[0] = XS - X2;
                ...
                if (XS < -0.5 || XS > 2) {
                    f_1 << "\nКОЭФФИЦИЕНТ СУММЫ СМЕЩЕНИЙ            XS " << setw(14) << round(XS * 1000) / 1000 << endl;
        ...
        ZPRGMS(ALFTW, GRM, SEC);
        if (IPR >= 3) {
            f_1 << "\nПЕРЕДАТОЧНОЕ ЧИСЛО                    U " << setw(14) << UU;
            f_1 << "\nУГОЛ ЗАЦЕПЛЕНИЯ (ГРАД.МИН.СЕК)    ALFTW " << setprecision(2) << fixed << setw(13) << GRM << "." << SEC << endl;
        }
