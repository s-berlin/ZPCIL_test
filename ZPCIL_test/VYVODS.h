...
void  VYVODS()
{
    ...
    ZPRGMS(BE, GRM, SEC);
    f_1 << "\nУГОЛ НАКЛОНА ЗУБА (ГРАД.МИН.СЕК)       BE      " << GRM << SEC;
    if(AW == 0)
    f_1 << "\nКОЭФФИЦИЕНТЫ СМЕЩЕНИЯ                  X  " << X1 << "      " << X2;
    if(AW != 0)
    f_1 << "\nМЕЖОСЕВОЕ РАССТОЯНИЕ                   AW      " << AW;

    f_1 << "\nШИРИНА ЗУБЧАТОГО ВЕНЦА                 B       " << B1 << "      " << B2;
    f_1 << "\nКОЭФФ. ВЫСОТЫ ГОЛОВКИ ЗУБА             HA      " << HA;
    f_1 << "\nНАЛИЧИЕ МОДИФИКАЦИИ ГОЛОВКИ ЗУБА       IMD     " << KM[IMD];  //IM не равно IMD !!! где задается IM ?????
    if(IST1 == IST2 && IST1 == IST3)
    f_1 << "\nСТЕПЕНЬ ТОЧНОСТИ ПО ГОСТ 1643-81       IST     " << IST1 << KT[IST4-1] << " - " << KT5[IST5-1];
    else
    f_1 << "\nСТЕПЕНЬ ТОЧНОСТИ ПО ГОСТ 1643-81       IST     " << IST1 << " - " << IST2 << " - " << IST3 << KT[IST4-1] << " - " << KT5[IST5-1];
    ...
    {
        f_1 << "\nПРИЗНАК РАСПОЛОЖЕНИЯ \nЗУБЧАТЫХ КОЛЕС                         IKG     " << IKG;
        
        if (IKG == 0)
        {
            f_1 << "\nОТКЛОНЕНИЕ ПОЛОЖЕНИЯ \nКОНТАКТНЫХ ЛИНИЙ                       FKE     " << FKE;
 ...
 //           f_1 << "\n  1 - одностороннее \n -1 - двухстороннее ";
            ...
            f_1 << "\nЧИСЛО ЗАЦЕПЛЕНИЙ ЗА ОБОРОТ             CZ      " << CZ1 << "      " << CZ2;
            f_1 << "\nПРИЗНАК НАЛИЧИЯ МАССИВНЫХ ДЕТЕЛЕЙ      GM      " << GM;
            f_1 << "\nПРИЗНАК ЗАДАНИЯ КОЭФФИЦИЕНТА ЗАПАСА          ";
            f_1 << "\nПРОЧНОСТИ ПО КОНТАКТНОЙ ВЫНОСЛИВ.      IVR     " << IVR;
         ...
         */
            ...
            f_1 << "\nПО ИЗГИБНОЙ ВЫНОСЛИВ.                  PR      " << PR;

            f_1 << "\nКОЭФФИЦИЕНТ ПОЛЕЗНОГО ДЕЙСТВИЯ         KPD     " << KPD;
            f_1 << "\nКОЭФФИЦИЕНТ СИЛОВОГО ПОТОКА            KSP     " << KSP;

            f_1 << "\nНОМЕР ГРУППЫ МАТЕРИАЛА                 IG      " << IG1 << "      " << IG2;
            f_1 << "\nТВЕРДОСТЬ ПОВЕРХНОСТИ                  H       " << H1 << "     " << H2;
            f_1 << "\nТВЕРДОСТЬ СЕРДЦЕВИНЫ                   HK      " << HK1 << "     " << HK2;
            f_1 << "\nТОЛЩИНА УПРОЧНЕННОГО СЛОЯ              HT      " << HT1 << "    " << HT2;
            f_1 << "\nШЕРОХОВАТОСТЬ ПОВЕРХНОСТИ ЗУБА         SHER    " << SHER1 << "    " << SHER2;
            f_1 << "\nКОЭФФ. СПОСОБА ПОЛУЧЕНИЯ ЗАГОТОВКИ     YZ      " << YZ1 << "      " << YZ2;
            f_1 << "\nТЕХНОЛОГИЧЕСКИЙ КОЭФФИЦИЕНТ            YT      " << YT1 << "      " << YT2;

            f_1 << "\nКОЭФФИЦИЕНТЫ ШЕРОХОВАТОСТИ    ";
            f_1 << "\nПЕРЕХОДНОЙ ПОВЕРХНОСТИ                 YR      " << YR1 << "      " << YR2;
            if (IMF1 != 0 || IMF2 != 0) {
                f_1 << "\nПРИЗНАК НАЛИЧИЯ ШЛИФОВАНИЯ    ";
                f_1 << "\nПЕРЕХОДНОЙ ПОВЕРХНОСТИ                 IMF     " << IMF1 << "     " << IMF2;
            }
            if (IVR == 1) {
                f_1 << "\nЗАДАВАЕМЫЙ КОЭФ. ЗАПАСА ПРОЧНОСТИ    ";
                f_1 << "\nПО КОНТАКТНОЙ ВЫНОСЛИВОСТИ             SHV     " << SHV1 << "     " << SHV2;
            }
            if (SHLM1 > 0 || SHLM2 > 0) {
                f_1 << "\nДОПУСКАЕМOЕ КОНТАКТНОЕ НАПРЯЖЕНИE    ";
                f_1 << "\nПРИ МАКСИМАЛЬНОЙ НАГРУЗКЕ " << IE2[I2] << "    SGHPM   " << SHPM1 << "    " << SHPM2;
            }
            if (SFLB1 > 0 || SFLB2 > 0) {
                f_1 << "\nБАЗОВЫЙ ПРЕДЕЛ ВЫНОСЛИВОСТИ    ";
                f_1 << "\nЗУБЬЕВ ПРИ ИЗГИБЕ         " << IE2[I2] << "    SGFLB   " << SFLB1 << "     " << SFLB2;
            }
            if (SFPM1 > 0 || SFPM2 > 0) {
                f_1 << "\nДОПУСКАЕМOЕ НАПРЯЖЕНИЕ ПРИ ИЗГИБЕ    ";
                f_1 << "\nЗУБА МАКСИМАЛЬНОЙ НАГРУЗКОЙ " << IE2[I2] << "  SGFPM   " << SFPM1 << "    " << SFPM2;
            }
            if (IE == 2) {
