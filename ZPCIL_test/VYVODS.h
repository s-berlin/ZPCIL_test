...
void  VYVODS()
{
    ...
    cout << "VYVODS: GRM = " << GRM << "   SEC = " << SEC << endl;

    if (HA != 1 || GRM != 20 || CZV != 0.2 || IPR >= 6)
        f_1 << "\nПАРАМЕТРЫ ИСХОДНОГО КОНТУРА" << "\n alfa = " << GRM << "." << SEC << "    ha* = " << HA << "    hl* = " << HL << "    c* = " << CZV << endl;

    if (L != 0)   // L = 0 если нет расчета на прочность (только геометрия)
    {
        f_1 << "\nПРИЗНАК РАСПОЛОЖЕНИЯ \nЗУБЧАТЫХ КОЛЕС                       IKG     " << IKG << endl;
        
        if (IKG == 0)
        f_1 << "\nОТКЛОНЕНИЕ ПОЛОЖЕНИЯ \nКОНТАКТНЫХ ЛИНИЙ                     FKE     " << FKE << endl;
        f_1 << "\nПРИЗНАК РАСПОЛОЖЕНИЯ ШЕСТЕРНИ        IZ1     " << IZ1 << endl;
    }
 /*
