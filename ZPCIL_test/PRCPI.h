...
//   ОЕВЮРЭ pЕГСКЭРЮРНБ pЮЯВЕРЮ МЮ ОpНВМНЯРЭ ОpХ ХГЦХАЕ           
             ...
             float YDLT, float YR1, float YR2, float YX1, float YX2, float NFE1, float NFE2)
{
    ...
    if (YN1 != 0 || YN2 != 0.) {
        ...
        f_1 << "\nпюявермше   мюопъфемхъ, " << IE2[I2] << "        SIGF " << setprecision(0) << fixed << setw(10) << SIGF1 << setw(10) << SIGF2 << endl;
    }
    if (PR > 0.) {
        f_1 << "дносяйюелше мюопъфемхъ, " << IE2[I2] << "       SIGFP " << setw(10) << SGFP1 << setw(10) << SGFP2 << endl;
    }
    else {
        f_1 << "\n пюявермше гюоюяш опнвмнярх, " << IE2[I2] << "       SFF " << setprecision(2) << fixed << setw(10) << SFF1 << setw(10) << SFF2 << endl;
        f_1 << "\n пюявермше бепнърмнярх мепюгпсьемхъ, " << IE2[I2] << "  PF " << setprecision(2) << fixed << setw(10) << PF1 << setw(10) << PF2 << endl;
    }
    f_1 << "\n      опнвмнярэ опх хгцхае люйяхлюкэмшни мюцпсгйни, " << endl;
    f_1 << "\nлюйяхлюкэмше мюопъфемхъ, " << IE2[I2] << "       SIGFM " << setprecision(0) << fixed << setw(10) << SIGFM1 << setw(10) << SIGFM2 << endl;

    if (PR > 0.) {
        f_1 << "дносяйюелше  мюопъфемхъ, " << IE2[I2] << "       SGFPM " << setw(10) << SGFPM1 << setw(10) << SGFPM2 << endl;
    }
    else {       
        f_1 << "\n пюявермше гюоюяш опнвмнярх, " << IE2[I2] << "       SFF " << setprecision(2) << fixed << setw(10) << SFMF1 << setw(10) << SFMF2 << endl;
        f_1 << "\n пюявермше бепнърмнярх мепюгпсьемхъ, " << IE2[I2] << "  PF " << setw(10) << PFM1 << setw(10) << PFM1 << endl;
    }
    if (IPR > 3) {
        f_1 << "\nпюявермши лнлемр мю бедсыел бюкс," << IE1[I1] << " T1F " << setprecision(0) << fixed << setw(8) << RTQ1 << setw(10) << RTQ2 << endl;
    }
    if (IPR > 3 && L <= -1.) {
        f_1 << "нйпсфмюъ яйнпнярэ опх пюявермни мюцпсгйе,Л/Я V " << setprecision(1) << fixed << setw(11) << VR << endl;
    }
    if (IPR > 3) {
        f_1 << "\n        оюпюлерпш пюявермнцн мюопъфемхъ опх хгцхае \nйнщттхжхемрш: \n";
        f_1 << "- тнплш гсаю                             YFS " << setprecision(3) << fixed << setw(10) << YF1 << setw(10) << YF2 << endl;
        f_1 << "- сцкю мюйкнмю                          YBET " << setw(15) << YBET << endl;
        f_1 << "- дкхмш йнмрюйрмшу кхмхи                YEPS " << setw(15) << YEPS << endl;
    }
    if (IPR > 3 && L >= -1.) {
        f_1 << "- дхмюлхвеяйни мюцпсгйх                  KFV " << setw(15) << KFV1 << endl;
    }
    if (IPR > 3) {
        f_1 << "- пюяопед.мюцпсгйх он дкхме йнмр.кхмхи   KFB " << setw(15) << KFB1 << endl;
        f_1 << "- пюяопедекемхъ мюцпсгйх лефдс гсаэълх   KFA " << setw(15) << KFA1 << endl;
    }
    if (IPR > 3) {
        f_1 << "\n        оюпюлерпш нцпюмхвеммнцн опедекю бшмнякхбнярх \nопедекш бшмнякхбнярх \n";
        f_1 << "аюгнбше, нрмскебше, " << IE2[I2] << "           SGFLB0 " << setprecision(0) << fixed << setw(10) << SGFLM1 << setw(10) << SGFLM2 << endl;
        f_1 << "опедекш онбпефдюелнярх, " << IE2[I2] << "         SGFG " << setw(10) << SGFG1 << setw(10) << SGFG2 << endl;
    }
    if (IPR > 3 && PR > 0.) {
        f_1 << "йнщттхжхемрш аегноюямнярх                 SF " << setprecision(3) << fixed << setw(10) << SF1 << setw(10) << SF2 << endl;
    }
    if (IPR > 3) {
        f_1 << "йнщттхжхемрш днкцнбевмнярх                YN " << setw(10) << YN1 << setw(10) << YN2 << endl;
        f_1 << "щйбхбюкемрмше вхякю жхйкнб, ЛКМ          NFE " << setw(10) << NFE1 << setw(10) << NFE2 << endl;
    }
    if (IPR > 3) {
        f_1 << "йнщттхжхемрш: \n";
        f_1 << "- дбсярнпнммецн опхкнфемхъ мюцпсгйх       YA " << setw(10) << YA1 << setw(10) << YA2 << endl;
        f_1 << "- цпюдхемрю мюопъфемхи                  YDLT " << setw(15) << YDLT << endl;
        f_1 << "- ьепнунбюрнярх оепеундмшу онбепумняре    YR " << setw(10) << YR1 << setw(10) << YR2 << endl;
        f_1 << "- пюглепю гсавюрнцн йнкеяю                YX " << setw(10) << YX1 << setw(10) << YX2 << endl;
    }
