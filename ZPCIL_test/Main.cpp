...
float LO, S1, FKE = 0, GM;               // расстояние между опорами, расстояние от опоры со стороны подвода момента до центра шестерни, ?, ?
int IQ, IP, IZ1 = 0, IG;                 // тип приложения момента, тип подшипника, ?, ?
float GRM = 0;
int SEC = 0;
