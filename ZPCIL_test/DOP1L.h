           ...
           float EWS1[2], float EWS2[2], float EWS[2], float TWM[2], float TW[2], float ECS[2], float TC[2])  //M, D, KST, KST4, KST5, FR, EHS, TH, EWS1, EWS2, EWS, TWM, TW, ECS, TC)
{
    ...
    int MFR[7][53] = { 6, 7, 8,  10,11,13,  16,18,20, 0,  25,28,32, 0,  36, 40, 45,  0,  0,  45, 50, 56,  0,  0,  0,   71, 80, 90,  0,  0,  0,  0,   100,125,140,  0,  0,  0,  0,  125,160,180,  0,  0,  0,  0,  160,200,224,  0,  0,  0,  0,    // 53 значения 
    ...
    //int LMEHS1[336] = 
    int MEHS[18][24] = { 12, 14,    30, 35,    46, 50, 55,    74, 80, 90, 100,   120, 140, 140, 160, 160, 180,    190, 200, 220, 250, 280, 280, 300,
    ...
    int MEWS1[12][24] = {  8, 10, 20,  25,  30,  35,  40,  50,  55,  60,  70,  80, 100, 100, 110, 110, 120, 120, 140, 160, 180, 200, 200, 200,
    ...
    int MTWM[21][8] = { 16,  20,  28,  35,   45,   60,   70,   90,
    ...
    int MTW[21][8] = { 20, 25, 35, 35, 50, 60, 80, 100,
    ...
    //int LMECS1[336] = 
    int MECS[18][24] = {9, 10,    22, 25,    35, 35, 40,    55,  60,  70,  70,  90, 100, 100, 120, 120, 140,     140, 150, 160, 180, 200, 200, 220,
    ...
    //    CCCC      if (D[i] <= 1600.) IDW = 12;
        ...
        cout << "DOD1L: IMF = " << IMF << "   IDF = " << IDF << endl;
 
        cout << "DOD1L: MFR[0][19] = " << MFR[1][19] << endl;

        cout << "DOD1L: IMF = " << IMF << "   IDF = " << IDF << "   MFR[IDF-1][IMF-1] = " << MFR[IDF - 1][IMF - 1] << endl;

        FR[i] = static_cast<float>(MFR[IDF - 1][IMF-1]);
        if (FR[i] <= 800.) IFR = 21;
        if (FR[i] <= 630) IFR = 20;
        if (FR[i] <= 500.) IFR = 19;
        if (FR[i] <= 400.) IFR = 18;
        ...
        cout << "DOD1L: IVS = " << IVS << "   IDE = " << IDE << "   MEHS[IDE-1][IVS-1] = " << MEHS[IDE - 1][IVS - 1] << endl;
