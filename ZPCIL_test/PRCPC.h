            ...
            float NHLIM1, float NHLIM2, float NHE1, float NHE2)
{
    ...
    f_1 << "\nРАСЧЕТНОЕ   НАПРЯЖЕНИЕ, " << IE2[I2] << "        SIGH " << setprecision(0) << fixed << setw(14) << SIGH << endl;

    if (BE == 0) {
        if (IVR <= 1) f_1 << "\nДОПУСКАЕМЫЕ НАПРЯЖЕНИЯ, " << IE2[I2] << "      SGHP " << setprecision(0) << fixed << setw(10) << SGHP1 << setw(10) << SGHP2 << endl;
        else {
    ...
    if (BE != 0.) {
        if (IVR <= 1) f_1 << "ДОПУСКАЕМOЕ НАПРЯЖЕНИE, " << IE2[I2] << "        SGHP " << setprecision(0) << fixed << setw(14) << SGHP << endl;
        else {
            f_1 << "\n РАСЧЕТНЫЕ ЗАПАСЫ ПРОЧНОСТИ " << "      SHF    " << SHF1 << "     " << SHF2 << endl;
            f_1 << "\n РАСЧЕТНЫЕ ВЕРОЯТНОСТИ НЕРАЗРУШЕНИЯ " << "   PH   " << PH1 << "     " << PH2 << endl;
        }
    }
    f_1 << "\n      КОНТАКТНАЯ ПРОЧНОСТЬ ПРИ ДЕЙСТВИИ МАКСИМАЛЬНОЙ НАГРУЗКИ" << endl;
    f_1 << "\nМАКСИМАЛЬНОЕ НАПРЯЖЕНИЕ," << IE2[I2] << "       SIGHM " << setprecision(0) << fixed << setw(14) << SIGHM;

    if (IVR <= 1) f_1 << "\nДОПУСКАЕМЫЕ НАПРЯЖЕНИЯ, " << IE2[I2] << "       SGHPM " << setw(10) << SGHPM1 << setw(10) << SGHPM2 << endl;
    else {
        f_1 << "\n РАСЧЕТНЫЕ ЗАПАСЫ ПРОЧНОСТИ " << "      SHF " << setw(10) << SHMF1 << setw(10) << SHMF2 << endl;
        f_1 << "\n РАСЧЕТНЫЕ ВЕРОЯТНОСТИ НЕРАЗРУШЕНИЯ " << "   PH " << setw(10) << PHM1 << setw(10) << PHM2 << endl;
    }
...
//           Пpовеpка на малоцикловую усталость
    ...
    if (IPR > 3)  f_1 << "\nРАСЧЕТНЫЙ МОМЕНТ НА ВЕДУЩЕМ ВАЛУ, " << IE1[I1] << "   T1H " << setw(11) << RTQ1 << endl;
 
    if (IPR > 3 && L >= - 1.) f_1 << "ОКРУЖНАЯ СКОРОСТЬ ПРИ РАСЧЕТНОЙ НАГРУЗКЕ,м/с V " << setprecision(1) << fixed << setw(11) << VR << endl;
  
    if (IPR > 6)  f_1 << "КОЭФФИЦИЕНТ НАГРУЖЕННОСТИ ПЕРЕДАЧИ, " << IE2[I2] << " K0 " << setprecision(2) << fixed << setw(11) << K01 << endl;

    if (IPR > 3) {
        f_1 << "\n     ПАРАМЕТРЫ РАСЧЕТНОГО КОНТАКТНОГО НАПРЯЖЕНИЯ " << endl;
        f_1 << "\nКОЭФФИЦИЕНТЫ: ";
        f_1 << "\n- ФОРМЫ СОПРЯЖЕННЫХ ПОВЕРХНОСТЕЙ         ZH " << setprecision(3) << fixed << setw(14) << ZH;
        f_1 << "\n- ДЛИНЫ КОНТАКТНЫХ ЛИНИЙ               ZEPS " << setprecision(3) << fixed << setw(14) << ZEPS;
    }
    if (IPR > 3 && L >= - 1.) f_1 << "\n- ДИНАМИЧЕСКОЙ НАГРУЗКИ                 KHV " << setw(14) << KHV1;

    if (IPR > 3 && KNR < 0)   f_1 << "\n- РАСПРЕД.НАГРУЗКИ МЕЖДУ САТЕЛЛИТАМИ OMEGA " << setw(14) << OMEG1;

    if (IPR > 3) {
        f_1 << "\n- РАСПРЕД.НАГРУЗКИ ПО ДЛИНЕ КОНТ.ЛИНИЙ  KHB " << setw(14) << KHB1;
        f_1 << "\n- РАСПРЕДЕЛЕНИЯ НАГРУЗКИ МЕЖДУ ЗУБЬЯМИ  KHA " << setw(14) << KHA1 << endl;
    }
    if (IPR > 3) {
        f_1 << "\n     ПАРАМЕТРЫ ОГРАНИЧЕННОГО ПРЕДЕЛА КОНТАКТНОЙ ВЫНОСЛИВОСТИ " << endl;
        f_1 << "\nПРЕДЕЛЫ КОНТАКТНОЙ ";
        f_1 << "\nВЫНОСЛИВОСТИ, " << IE2[I2] << "                SGMHLB " << setprecision(0) << fixed << setw(10) << SGHLM1 << setw(10) << SGHLM2;
        f_1 << "\nПОВРЕЖДАЮЩИЕ НАПРЯЖЕНИЯ, " << IE2[I2] << "       SGHG " << setw(10) << SGHG1 << setw(10) << SGHG2;
    }

        if (IPR > 3 && IVR == 0) 
            f_1 << "\nКОЭФФИЦИЕНТЫ БЕЗОПАСНОСТИ                SH " << setprecision(3) << fixed << setw(10) << SH1 << setw(10) << SH2;

        if (IPR > 3) {
            f_1 << "\nКОЭФФИЦИЕНТЫ ДОЛГОВЕЧНОСТИ               ZN " << setw(10) << ZN1 << setw(10) << ZN2;
            f_1 << "\nКОЭФФИЦИЕНТЫ ОКРУЖНОЙ СКОРОСТИ           ZV " << setw(10) << ZV1 << setw(10) << ZV2;
            f_1 << "\nКОЭФФИЦИЕНТЫ РАЗМЕРА ЗУБЧАТОГО КОЛЕСА    ZX " << setw(10) << ZX1 << setw(10) << ZX2;

            f_1 << "\nКОЭФФИЦИЕНТ ШЕРОХОВАТОСТИ  ";
            f_1 << "\nРАБОЧИХ ПОВЕРХНОСТЕЙ                     ZR " << setw(14) << ZR;

            f_1 << "\nБАЗОВЫЕ ЧИСЛА ЦИКЛОВ, МЛН             NHLIM " << setw(10) << NHLIM1 << setw(10) << NHLIM2;
            f_1 << "\nЭКВИВАЛЕНТНЫЕ ЧИСЛА ЦИКЛОВ, МЛН         NHE " << setw(10) << NHE1 << setw(10) << NHE2;
        }
